...
export class SqlExecutorService {
  ...
  execute(query: string): SqlResponse {
    if (!this.alasql) return { error: 'Database not initialized' };
    try {
      ...
      return { result: { rows: results, columns } };
    } catch (e: any) {
      return { error: e.message || 'An error occurred during query execution.' };
    }
  }
}
