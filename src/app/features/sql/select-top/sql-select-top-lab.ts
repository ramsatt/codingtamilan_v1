...
@Component({
  selector: 'app-sql-select-top-lab',
  imports: [CommonModule, FormsModule],
  template: `
    <section id="sql-select-top" class="scroll-mt-8 space-y-8">
      <div>
        <h3 class="text-3xl font-bold text-slate-800 mb-4 border-l-4 border-indigo-500 pl-4">SQL SELECT TOP / LIMIT</h3>
        <p class="article-body">
          The <span class="font-mono font-bold text-indigo-600">LIMIT</span> clause is used to specify the number of records to return.
        </p>
      </div>

      <div class="glass-panel rounded-xl p-6 shadow-sm bg-white border border-slate-100">
        <h4 class="text-xl font-bold text-slate-800 mb-4">LIMIT Syntax</h4>
        <div class="bg-slate-900 text-slate-100 p-4 rounded-lg font-mono text-sm">
          ...
          <div><span class="text-indigo-400">LIMIT</span> number;</div>
        </div>
        <p class="text-slate-600 mt-3 text-sm">Note: Different databases use different syntax (TOP, LIMIT, FETCH FIRST, ROWNUM)</p>
      </div>

      <div class="glass-panel rounded-xl p-6 shadow-sm bg-white border border-slate-100">
        <h4 class="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
          <span class="p-1 bg-teal-100 text-teal-700 rounded text-xs">Interactive</span> Try LIMIT
        </h4>
        ...
        @if (results().length > 0) {
          <div class="mt-6">
            <h5 class="font-bold text-slate-700 mb-3">Limited Results ({{ results().length }} rows)</h5>
...
export class SqlSelectTopLab implements OnInit {
  sqlService = inject(SqlExecutorService);
  
  sqlQuery = signal('SELECT * FROM Customers LIMIT 5;');
  results = signal<any[]>([]);
  ...
  examples = [
    {
      title: 'First 5 Records',
      description: 'Get the first 5 customers',
      query: 'SELECT * FROM Customers LIMIT 5;'
    },
    {
      title: 'Top 3 Products',
      description: 'Get 3 products',
...
LIMIT 5;`
    },
    {
      ...
      description: 'Skip first 5, get next 5 (pagination)',
      query: 'SELECT * FROM Customers LIMIT 5 OFFSET 5;'
    }
  ];
  ...
  resetQuery() {
    this.sqlQuery.set('SELECT * FROM Customers LIMIT 5;');
    this.results.set([]);
    this.error.set('');
  }
