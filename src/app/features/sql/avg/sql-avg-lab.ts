...
@Component({
  selector: 'app-sql-avg-lab',
  imports: [CommonModule, FormsModule],
  template: `
    <section id="sql-avg" class="scroll-mt-8 space-y-8">
      <div>
        <h3 class="text-3xl font-bold text-slate-800 mb-4 border-l-4 border-lime-500 pl-4">SQL AVG() Function</h3>
        <p class="article-body">
          The <span class="font-mono font-bold text-lime-600">AVG()</span> function returns the average value of a numeric column.
        </p>
      </div>

      <div class="glass-panel rounded-xl p-6 shadow-sm bg-white border border-slate-100">
        <h4 class="text-xl font-bold text-slate-800 mb-4">AVG() Syntax</h4>
        <div class="bg-slate-900 text-slate-100 p-4 rounded-lg font-mono text-sm">
          <div><span class="text-pink-400">SELECT AVG</span>(column_name)</div>
      ...
      <div class="glass-panel rounded-xl p-6 shadow-sm bg-white border border-slate-100">
        <h4 class="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
          <span class="p-1 bg-teal-100 text-teal-700 rounded text-xs">Interactive</span> Try AVG()
        </h4>
              ...
              <table class="w-full text-sm">
                ...
                <tbody class="bg-white">
                  @for (row of results(); track $index) {
                    <tr>
                      @for (col of columns(); track col) {
                        <td class="border-b border-slate-100 p-3 text-2xl font-bold text-lime-600">{{ row[col] }}</td>
                      }
                    </tr>
                  }
                </tbody>
...
export class SqlAvgLab implements OnInit {
  sqlService = inject(SqlExecutorService);
  
  sqlQuery = signal('SELECT AVG(Price) AS AveragePrice FROM Products;');
  results = signal<any[]>([]);
  ...
  examples = [
    ...
    {
      title: 'Average with WHERE',
      description: 'Average price for specific category',
      query: 'SELECT AVG(Price) AS AvgPrice FROM Products WHERE CategoryID = 1;'
    },
    {
      title: 'Average with GROUP BY',
...
GROUP BY CategoryID;`
    },
    {
      ...
      description: 'Round average to 2 decimal places',
      query: 'SELECT ROUND(AVG(Price), 2) AS AvgPrice FROM Products;'
    }
  ];
  ...
  resetQuery() {
    this.sqlQuery.set('SELECT AVG(Price) AS AveragePrice FROM Products;');
    this.results.set([]);
    this.error.set('');
  }
