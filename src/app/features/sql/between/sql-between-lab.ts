...
@Component({
  selector: 'app-sql-between-lab',
  imports: [CommonModule, FormsModule],
  template: `
    <section id="sql-between" class="scroll-mt-8 space-y-8">
      <div>
        <h3 class="text-3xl font-bold text-slate-800 mb-4 border-l-4 border-rose-500 pl-4">SQL BETWEEN Operator</h3>
        <p class="article-body">
          The <span class="font-mono font-bold text-rose-600">BETWEEN</span> operator selects values within a given range (inclusive).
        </p>
      </div>

      <div class="glass-panel rounded-xl p-6 shadow-sm bg-white border border-slate-100">
        <h4 class="text-xl font-bold text-slate-800 mb-4">BETWEEN Syntax</h4>
        <div class="bg-slate-900 text-slate-100 p-4 rounded-lg font-mono text-sm">
          ...
          <div><span class="text-pink-400">WHERE</span> column_name <span class="text-rose-400">BETWEEN</span> value1 <span class="text-rose-400">AND</span> value2;</div>
        </div>
      ...
      <div class="glass-panel rounded-xl p-6 shadow-sm bg-white border border-slate-100">
        <h4 class="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
          <span class="p-1 bg-teal-100 text-teal-700 rounded text-xs">Interactive</span> Try BETWEEN
        </h4>
...
export class SqlBetweenLab implements OnInit {
  sqlService = inject(SqlExecutorService);
  
  sqlQuery = signal(`SELECT * FROM Products
WHERE Price BETWEEN 10 AND 20;`);
  ...
  examples = [
    {
      title: 'Price Range',
      description: 'Products with price between 10 and 20',
      query: `SELECT * FROM Products
WHERE Price BETWEEN 10 AND 20;`
    },
    {
      title: 'NOT BETWEEN',
      description: 'Products outside the price range',
      query: `SELECT * FROM Products
WHERE Price NOT BETWEEN 10 AND 20;`
    },
    {
      title: 'Text Range',
      description: 'Products alphabetically between C and M',
      query: `SELECT * FROM Products
...
ORDER BY ProductName;`
  ...
  resetQuery() {
    this.sqlQuery.set(`SELECT * FROM Products
WHERE Price BETWEEN 10 AND 20;`);
    this.results.set([]);
    this.error.set('');
  }
