...
@Component({
  selector: 'app-sql-union-lab',
  imports: [CommonModule, FormsModule],
  template: `
    <section id="sql-union" class="scroll-mt-8 space-y-8">
      <div>
        <h3 class="text-3xl font-bold text-slate-800 mb-4 border-l-4 border-purple-500 pl-4">SQL UNION Operator</h3>
        <p class="article-body">
          The <span class="font-mono font-bold text-purple-600">UNION</span> operator is used to combine the result-set of two or more SELECT statements.
        </p>
      </div>

      <div class="glass-panel rounded-xl p-6 shadow-sm bg-white border border-slate-100">
        <h4 class="text-xl font-bold text-slate-800 mb-4">UNION Syntax</h4>
        <div class="bg-slate-900 text-slate-100 p-4 rounded-lg font-mono text-sm">
          <div><span class="text-pink-400">SELECT</span> column_name(s) <span class="text-pink-400">FROM</span> table1</div>
          <div><span class="text-purple-400">UNION</span></div>
          <div><span class="text-pink-400">SELECT</span> column_name(s) <span class="text-pink-400">FROM</span> table2;</div>
        </div>
        <div class="mt-4 p-4 bg-purple-50 border border-purple-200 rounded-lg">
          <p class="text-purple-700 font-medium">💡 UNION Rules:</p>
          <ul class="text-purple-600 text-sm mt-2 space-y-1">
            <li>• Every SELECT must have the same number of columns</li>
            <li>• Columns must have similar data types</li>
            <li>• Columns must be in the same order</li>
            <li>• UNION selects only DISTINCT values by default</li>
          </ul>
        </div>
      </div>
      ...
      <div class="glass-panel rounded-xl p-6 shadow-sm bg-white border border-slate-100">
        <h4 class="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
          <span class="p-1 bg-teal-100 text-teal-700 rounded text-xs">Interactive</span> Try UNION
        </h4>
      ...
      <div class="glass-panel rounded-xl p-6 shadow-sm bg-purple-50 border border-purple-200">
        <h4 class="text-xl font-bold text-purple-800 mb-3">UNION vs UNION ALL</h4>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="bg-white p-4 rounded border border-purple-200">
            <h5 class="font-semibold text-slate-800 mb-2">UNION</h5>
            <p class="text-sm text-slate-600">Returns only DISTINCT values (removes duplicates)</p>
          </div>
          <div class="bg-white p-4 rounded border border-purple-200">
            <h5 class="font-semibold text-slate-800 mb-2">UNION ALL</h5>
...
export class SqlUnionLab implements OnInit {
  ...
  examples = [
    {
      ...
      description: 'Get unique cities from Customers and Suppliers',
...
ORDER BY City;`
    ...
    {
      title: 'UNION with WHERE',
      description: 'Filter before combining',
      query: `SELECT City, Country FROM Customers WHERE Country='Germany'
UNION
...
ORDER BY City;`
    ...
    {
      title: 'UNION ALL',
      description: 'Include duplicate values',
      query: `SELECT City FROM Customers
UNION ALL
SELECT City FROM Suppliers
ORDER BY City;`
    ...
    {
      title: 'UNION with Aliases',
      description: 'Add source identifier',
      query: `SELECT 'Customer' AS Type, CustomerName AS Name, City FROM Customers
UNION
SELECT 'Supplier', CompanyName, City FROM Suppliers;`
    }
  ];
