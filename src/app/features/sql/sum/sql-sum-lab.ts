...
@Component({
  selector: 'app-sql-sum-lab',
  imports: [CommonModule, FormsModule],
  template: `
    <section id="sql-sum" class="scroll-mt-8 space-y-8">
      <div>
        <h3 class="text-3xl font-bold text-slate-800 mb-4 border-l-4 border-orange-500 pl-4">SQL SUM() Function</h3>
        <p class="article-body">
          The <span class="font-mono font-bold text-orange-600">SUM()</span> function returns the total sum of a numeric column.
        </p>
      </div>

      <div class="glass-panel rounded-xl p-6 shadow-sm bg-white border border-slate-100">
        <h4 class="text-xl font-bold text-slate-800 mb-4">SUM() Syntax</h4>
        <div class="bg-slate-900 text-slate-100 p-4 rounded-lg font-mono text-sm">
          <div><span class="text-pink-400">SELECT SUM</span>(column_name)</div>
      ...
      <div class="glass-panel rounded-xl p-6 shadow-sm bg-white border border-slate-100">
        <h4 class="text-xl font-bold text-slate-800 mb-4 flex items-center gap-2">
          <span class="p-1 bg-teal-100 text-teal-700 rounded text-xs">Interactive</span> Try SUM()
        </h4>
              ...
              <table class="w-full text-sm">
                ...
                <tbody class="bg-white">
                  @for (row of results(); track $index) {
                    <tr>
                      @for (col of columns(); track col) {
                        <td class="border-b border-slate-100 p-3 text-2xl font-bold text-orange-600">{{ row[col] }}</td>
                      }
                    </tr>
                  }
                </tbody>
...
export class SqlSumLab implements OnInit {
  sqlService = inject(SqlExecutorService);
  
  sqlQuery = signal('SELECT SUM(Quantity) AS TotalQuantity FROM OrderDetails;');
  results = signal<any[]>([]);
  ...
  examples = [
    {
      title: 'Sum All Quantities',
      description: 'Calculate total quantity from all order details',
      query: 'SELECT SUM(Quantity) AS TotalQuantity FROM OrderDetails;'
    },
    {
      title: 'Sum with WHERE',
      description: 'Sum quantities for a specific order',
      query: 'SELECT SUM(Quantity) AS OrderTotal FROM OrderDetails WHERE OrderID = 10248;'
    },
    {
      title: 'Sum with GROUP BY',
      description: 'Sum quantities per order',
      query: `SELECT OrderID, SUM(Quantity) AS Total
FROM OrderDetails
GROUP BY OrderID;`
    }
  ];
  ...
  resetQuery() {
    this.sqlQuery.set('SELECT SUM(Quantity) AS TotalQuantity FROM OrderDetails;');
    this.results.set([]);
    this.error.set('');
  }
